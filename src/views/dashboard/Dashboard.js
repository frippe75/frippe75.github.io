...
import { useHistory, Redirect, Route, Router } from 'react-router-dom';
...
class Dashboard extends Component {
  ...
  constructor(props) {
    ...
    request.onupgradeneeded = function (e){
      ...
      if (e.oldVersion == 0) {
        console.log("TermBeeDB database does not exist, init and redirect to Setup Guide");
        initDB(DBConfig);
  ...
  componentDidMount() {
    console.log("Dashboard: In App CompnentDidMount")
  ...
  render() {
    ...
    return (
      <>
        <WidgetsDropdown />
        <h1>State is { String(this.state.DBcreated) }</h1> 
